...
PRM_Template SOP_Cam_Frustum::myTemplateList[] = {
    PRM_Template(PRM_STRING, PRM_TYPE_DYNAMIC_PATH, 1, &PRMcameraPath, &camPathDefault, 0, 0, 0,
...
OP_Node *SOP_Cam_Frustum::myConstructor(OP_Network *net, const char *name, OP_Operator *op) {
...
void SOP_Cam_Frustum::buildPoly(GU_Detail *dst, UT_Vector3RArray &pts) {
...
bool SOP_Cam_Frustum::updateParmsFlags() {
...
void SOP_Cam_Frustum::buildFrustum(GU_Detail *dst, OP_Context &context, OBJ_Node *camera_node) {
  ...
  // UT_Vector3 up_right =
  // UT_Vector3(aperture*0.5/focal,(resy*aperture)/(resx*aspect)*0.5/focal,-1);
  //(ch("../../cam1/resy")*ch("../../cam1/aperture")) /
  //(ch("../../cam1/resx")*ch("../../cam1/aspect")) * 0.5 far frustum points
  const UT_Vector3 up_right_far =
      UT_Vector3(aperture * 0.5, (resy * aperture) / (resx * aspect) * 0.5, -focal) *
      FARDISTFROMCAM(now) / focal;
  ...
  // near frustum points
  const UT_Vector3 up_right_near =
      UT_Vector3(aperture * 0.5, (resy * aperture) / (resx * aspect) * 0.5, -focal) *
      NEARDISTFROMCAM(now) / focal;
